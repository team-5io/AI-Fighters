...
3. 같은 경로에 `.env` 파일 생성 — `DATABASE_URL`, `GEMINI_API_KEY` 등 [.env.example](../.env.example) 기준으로 실제 값 채워서. 이 파일은 서버에만 두고 git에는 절대 올리지 않는다.
4. GitHub Actions 배포용 SSH 키 페어 생성 → **공개키는 EC2 `~/.ssh/authorized_keys`에, 개인키는 아래 GitHub Secrets에**
5. 배포 대상 DB(RDS든 EC2 내 컨테이너든)가 `DATABASE_URL`에서 실제로 붙는지 확인
