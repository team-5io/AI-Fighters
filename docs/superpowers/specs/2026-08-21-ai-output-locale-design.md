...
- 작성일: 2026-08-21
- 상태: **폐기 (SUPERSEDED)** — 2026-08-21 팀 결정으로 방향이 바뀌었다

> ## ⚠️ 이 설계는 채택되지 않았다
>
> **결정 (2026-08-21)**: AI는 **항상 영어로 생성**하고, 사용자 언어로의 번역은
> **FE 브라우저 온디바이스 번역**이 담당한다.
>
> 그 결과 이 문서의 다음 내용이 전부 무효다.
>
> - `locale` 전달 계약(3절), 공용 locale 모듈(4절), 생성 시점 언어 적용(5절)
> - 조회 시 번역(6절), `source_locale`·`ai_text_translation`·마이그레이션(6·7절)
> - BE의 `Accept-Language` 초기값 처리(13절)
> - D1~D9, D13
>
> **유효하게 남은 것**은 locale과 무관한 결정뿐이다.
>
> - D10 `locationRef` = `{"blockId","quote"}` (14.1절)
> - D11 제안 평평한 리스트 + 서버 정렬 (14.2절)
> - D12 DocumentLion·CIO 모델 설정 분리 (14.3절)
>
> **아이러니**: 11.1절이 온디바이스 번역을 "채택하지 않는다"고 기록하고 있다.
> 그 조사(Safari·모바일 미지원, glossary 없음, user activation 필요)는 사실이고 지금도
> 유효하다. 팀은 그 제약을 알고도 온디바이스를 택했다. **같은 조사를 다시 하지 말고
> 11.1절을 먼저 읽을 것.**
>
> 구현 이력: PR #27(구현) → #28(배포) → 이 방향 전환으로 되돌림.
